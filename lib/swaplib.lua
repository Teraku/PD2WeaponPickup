...
_G.WeaponSwap = WeaponSwap or class()
...
function WeaponSwap:set_weapon(t_weapon_id, t_blueprint, _config)
  ...
  if not t_factory_id or not t_blueprint then
    ...
    log("Invalid weapon id and/or blueprint!")
    return
  end
...
_G.InGame_WeaponSwap = InGame_WeaponSwap or class(WeaponSwap)
...
function InGame_WeaponSwap:set_weapon( ... ) -- '...' means take a variable amount of arguments, we could have alternatively typed the same parameters as the original function
    if not game_state_machine or not managers.player:player_unit() then
        log("Game state machine not set or player unit not set")
        return
    end --prevent crash when running this and not in-game.
  ...
  log("Calling base set_weapon func")
